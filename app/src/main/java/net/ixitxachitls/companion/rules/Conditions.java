...
public class Conditions {
  ...
  private static ImmutableList<Condition> CONDITIONS = new ImmutableList.Builder<Condition>()
      ...
      .add(new Condition("Grappling",
          "Engaged in wrestling or some other form of handto-hand struggle with one or more "
  ...
  public static List<BaseCreature.TimedCondition> asTimedConditions() {
    return CONDITIONS.stream()
        .map(c -> new BaseCreature.TimedCondition(c.name, c.description))
        .collect(Collectors.toList());
  }

  public static List<String> getNames() {
    return CONDITIONS.stream().map(c -> c.name).collect(Collectors.toList());
  }
}
